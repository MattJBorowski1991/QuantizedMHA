...
// Include the kernel implementation (defines solve)
#include "../mha_kernels/fa.cu"

static void check_cuda(cudaError_t e, const char* msg){ if(e != cudaSuccess){ std::cerr<<msg<<": "<<cudaGetErrorString(e)<<"\n"; exit(1);} }

int main(){
    // Load golden inputs and outputs from tests/golden/small
    ...
    // Use known small-case dims (match generate_golden)
    ...
    std::cout<<"device_test_fa: maxerr="<<maxerr<<"\n";
