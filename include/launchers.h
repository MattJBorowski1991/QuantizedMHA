...
//           int N, int d, float inv_sqrt_d, cudaStream_t stream, void* aux)
            ...
            int N, int d_model, int h, KernelFn kernel_fn, size_t aux_bytes_per_stream = 0)
{
    ...
    for (int head = 0; head < h; ++head) {
        ...
        kernel_fn(q_s, k_s, v_s, out_s, N, d, inv_sqrt_d, streams[s], aux_ptr);

        launch_concat_mat<TILE>(output, out_s, 0, curr_col, N, d_model, N, d, streams[s]);
    }
