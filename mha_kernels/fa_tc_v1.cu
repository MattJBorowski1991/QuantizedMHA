...
__global__ void fa_kernel(
    ...
    for (int kv_block_idx = 0; kv_block_idx < num_kv_blocks; kv_block_idx++) {
        ...
        // // DEBUG: Print raw scores after matmul (only thread 0 of block 0 at first KV iteration)
        // if (tid == 0 && q_block_idx == 0 && kv_block_idx == 0) {
        //     // Use a single printf with all data to avoid buffering issues
        //     printf("[fa_tc_v1] row0: %.1f %.1f %.1f %.1f %.1f %.1f %.1f %.1f | %.1f %.1f %.1f %.1f %.1f %.1f %.1f %.1f\n",
        //         scores[0 * (Bc + PAD) + 0], scores[0 * (Bc + PAD) + 1], scores[0 * (Bc + PAD) + 2], scores[0 * (Bc + PAD) + 3],
        //         scores[0 * (Bc + PAD) + 4], scores[0 * (Bc + PAD) + 5], scores[0 * (Bc + PAD) + 6], scores[0 * (Bc + PAD) + 7],
        //         scores[0 * (Bc + PAD) + 8], scores[0 * (Bc + PAD) + 9], scores[0 * (Bc + PAD) + 10], scores[0 * (Bc + PAD) + 11],
        //         scores[0 * (Bc + PAD) + 12], scores[0 * (Bc + PAD) + 13], scores[0 * (Bc + PAD) + 14], scores[0 * (Bc + PAD) + 15]);
        // }
        // __syncthreads();
